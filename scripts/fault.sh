...
inject() {
  ...
  if kubectl --context "$GKE_CONTEXT" --namespace "$PLATFORM_NAMESPACE" \
    get virtualservice platform-api --output json |
    jq -e '.spec.http[1].fault.abort' >/dev/null; then
    echo "GKE application fault is already active" >&2
    exit 1
  fi
  ...
  kubectl --context "$GKE_CONTEXT" --namespace "$PLATFORM_NAMESPACE" \
    patch virtualservice platform-api --type=json --patch "[
      {\"op\":\"add\",\"path\":\"/spec/http/1/fault\",\"value\":{
        \"abort\":{\"httpStatus\":503,\"percentage\":{\"value\":100}}
      }}
    ]" >/dev/null
  echo "GKE application-route HTTP 503 fault is active. Run make fault-load, then make fault-capture."
}
...
clear() {
  ...
  if ! kubectl --context "$GKE_CONTEXT" --namespace "$PLATFORM_NAMESPACE" \
    get virtualservice platform-api --output json |
    jq -e '.spec.http[1].fault.abort' >/dev/null; then
    echo "GKE application fault is already clear."
    return 0
  fi
  ...
  echo "GKE application-route HTTP 503 fault is clear."
}
...
status() {
  ...
  kubectl --context "$GKE_CONTEXT" --namespace "$PLATFORM_NAMESPACE" \
    get virtualservice platform-api --output json |
    jq '{active: (.spec.http[1].fault.abort != null), http_status: .spec.http[1].fault.abort.httpStatus}'
}
...
capture() {
  ...
  alert_response="$(curl --fail --silent --show-error --get \
    --data-urlencode 'query=ALERTS{alertname="PlatformApiAvailabilityDemo",alertstate="firing"}' \
    "http://127.0.0.1:${PROMETHEUS_LOCAL_PORT}/api/v1/query")"
  error_response="$(curl --fail --silent --show-error --get \
    --data-urlencode 'query=sum(rate(istio_requests_total{reporter="source",response_code="503"}[1m]))' \
    "http://127.0.0.1:${PROMETHEUS_LOCAL_PORT}/api/v1/query")"
  jq -n --argjson alerts "$alert_response" --argjson errors "$error_response" '
    {
      alert_firing: ($alerts.data.result | length > 0),
      http_503_requests_per_second: ($errors.data.result[0].value[1] // null)
    }
  '
}
