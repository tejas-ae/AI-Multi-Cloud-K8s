...
I keep the Argo CD bootstrap files here. The bootstrap script installs the pinned chart, resets any stale release values, uses Argo CD's native in-cluster target, confirms the settings ConfigMap exists, and adds the `platform-bootstrap` application.
